...
type commentOnPageItem struct {
	*models.Comment
	SubComments []*models.Comment `json:"sub_comments"`
	HasNext     bool              `json:"has_next"`
}
type commentOnPageList struct {
	Data    []*commentOnPageItem `json:"data"`
	HasNext bool                 `json:"has_next"`
}

func commentPageListRoute(app *core.App) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		article_id, err := strconv.ParseUint(chi.URLParam(r, "articleID"), 10, 32)
		core.P(err)
		query := r.URL.Query()
		...
		if enableCache {
			data := app.Cache.Get(getCommentIndexCacheKey(int(article_id)))
			if data != nil {
				app.JSON(w, data)
				return
			}
		...
		pageSize := 5
		var count int64
		model := app.O.Model(&models.Comment{}).
		...
		pageComments := make([]*commentOnPageItem, len(comments))
		for i, c := range comments {
			...
			var subCount int64
			if rootId == 0 {
				subModel := app.O.Model(&models.Comment{}).
					Where("root_comment_id = ?", c.ID)
				err = subModel.Count(&subCount).Error
				core.P(err)
				if subCount == 0 {
					pageComments[i] = &commentOnPageItem{
						Comment:     c,
		...
		if enableCache {
			...
			app.Cache.Set(getCommentIndexCacheKey(int(article_id)), data, 0)
		}
		app.JSON(w, data)
	}
}
